...
export class BlogsService {
  ...
  async getAllBlogs(page?: string | number, limit?: string | number) {
    try {
      ...
      const [blogs, total] = await this.blogsRepository
        .createQueryBuilder('blog')
        .where('blog.status IN (:...statuses)', {
          statuses: this.approvedBlogStatuses,
        })
  ...
  async getUsersBlogs(
    ...
    try {
      ...
      const [blogs, total] = await this.blogsRepository
        ...
        .andWhere('blog.status IN (:...statuses)', {
          statuses: this.approvedBlogStatuses,
        })
        .orderBy('blog.createdAt', 'DESC')
        .skip(skip)
  ...
  async getPendingBlogsPerAuthor(
    req: any,
    page?: string | number,
    limit?: string | number,
  ) {
    ...
    try {
      ...
      if (file) {
        if (blog.coverImageId)
          await this.cloudinaryService.deleteImage(blog.coverImageId);
        ...
        const coverImageId = cloudinaryResponse.public_id;
        dto.coverImage = coverImage;
        dto.coverImageId = coverImageId;
      }

      if (dto.removeCoverImage && blog.coverImageId)
        await this.cloudinaryService.deleteImage(blog.coverImageId);
  ...
  async deleteBlog(id: string, coverImageId: string, req: any) {
    try {
      ...
      const response = await this.cloudinaryService.deleteImage(
        coverImageId,
      );
  ...
  async getRecommendedBlogs(req: any, page = 1, limit = 10) {
    try {
      const userId = req.user?.id;

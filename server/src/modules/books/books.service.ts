...
export class BooksService {
  ...
  async getPendingBooksPerAuthor(
    req: any,
    page?: string | number,
    limit?: string | number,
  ) {
    ...
    try {
      ...
      if (files?.length) {
        ...
        const cloudinaryResponses = await Promise.all(
          files.map((file) =>
            this.cloudinaryService.uploadImage(file, organization, folder),
          ),
        );
  ...
  async updateBook(id: string, dto: UpdateBookDto, req: any, files?: any[]) {
    try {
      ...
      const shouldRemoveExistingImages = Boolean(
        dto.removeImages || files?.length,
      );
      ...
      if (files?.length) {
        ...
        updatedPayload.images = [...(book.images || []), ...images];
    ...
    try {
      ...
      if (remainingAfterInterest > 0) {
        trendingBooks = await this.booksRepository
          ...
          .andWhere(interestBooks.length ? 'book.id NOT IN (:...ids)' : '1=1', {
      ...
      if (finalRemaining > 0) {
        fallbackBooks = await this.booksRepository
          .createQueryBuilder('book')
          .where('book.approved = true')
          .andWhere(combined.length ? 'book.id NOT IN (:...ids)' : '1=1', {

...
export class UserService {
  ...
  async login(dto: LoginDto, res: any) {
    try {
      const identifier =
        dto.identifier ?? (dto as any).email ?? (dto as any).username;
      ...
      return {
        ...
        user: {
          ...
          followersCount:
            user.role === 'author' ? user.followersCount : undefined,
          followingCount: user.followingCount,
          role: user.role,
          bio: user.bio ?? undefined,
          booksId: user.role === 'author' ? user.booksId : undefined,
          blogsId: user.role === 'author' ? user.blogsId : undefined,

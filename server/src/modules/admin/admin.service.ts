...
>>>>>>> Stashed changes
  ...
  async approve(req: any, bookId: string, approve: boolean) {
    try {
      ...
      const book = await this.bookRepository
        .createQueryBuilder('books')
        .where('books.id = :bookId', { bookId })
        .select(['books.id', 'books.approved', 'books.state'])
        .getOne();
  ...
  async advertiseBook(req: any, bookId: string, advertise: boolean) {
    try {
      ...
      if (advertise && !book.isAdvertised) {
        const advertisedCount = await this.bookRepository.count({
          where: { isAdvertised: true },
        });
        if (advertisedCount >= 5) {
          throw new BadRequestException({
            success: false,
            message: 'Maximum of 5 books can be advertised at a time',
          });
        }
      }
      ...
      return {
        success: true,
        message: advertise
          ? 'Book is now advertised'
          : 'Book advertisement removed',
        book,
      };
    } catch (error) {
      throw this.handleServiceError(error);
  ...
  async softDeleteBlog(req: any, blogId: string) {
    try {
      ...
      const deletedBlog = await this.blogRepository.softDelete({
        id: blogId,
      });
  ...
  async addPublisher(req: any, dto: CreatePublisherDto) {
    try {
      ...
      const existingPublisher = await this.publisherRepository.findOne({
        where: [{ publisherId: dto.publisherId }, { email: dto.email }],
      });

      if (existingPublisher) {
        throw new ConflictException({
          success: false,
          message: 'Publisher with this ID or email already exists',
        });
      }
  ...
  async updatePublisher(
    ...
    try {
      ...
      if (dto.publisherId || dto.email) {
        throw new BadRequestException({
          success: false,
          message: 'publisherId and email cannot be updated',
        });
      }

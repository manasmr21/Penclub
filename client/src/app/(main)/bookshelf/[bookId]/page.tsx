...
export default function BookDetailsPage() {
  ...
  useEffect(() => {
    ...
    const loadDetails = async () => {
      ...
      try {
        ...
        const directName =
          bookData?.author?.name?.trim() || bookData?.author?.username?.trim();
        if (directName) {
  ...
  const backLabel =
    from === "profile" ? "Back to profile" : "Back to bookshelf";
  ...
  const handleFollowAuthor = async () => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Unable to follow author.";
      alert(message);
    } finally {
        ...
        <div className="lg:col-span-9">
          ...
          <div className="space-y-5">
            {reviews.length ? (
              reviews.map((review) => {
                const reviewerName =
                  review.user?.name || review.user?.username || "Reader";
                return (
                  <article
                    key={review.id}
                  ...
                  >
                    ...
                    <p className="leading-relaxed text-muted-foreground">
                      {review.content?.trim() ||
                        "No written comment shared for this rating."}
                    </p>
                  </article>
                );
              })

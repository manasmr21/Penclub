...
import { fetchAuthorArticlesCount, fetchAuthorBooksCount } from "@/src/lib/profile-stats-api";
...
const UserDetails = () => {
  ...
  const hasProfilePicture =
    typeof user?.profilePicture === "string" && user.profilePicture.trim().length > 0;
  ...
  const parsedInterests = Array.isArray(user?.interests)
    ...
    : typeof user?.interests === "string"
      ? user.interests.split(",").map((interest: string) => interest.trim()).filter(Boolean)
      : [];
      ...
      <div className="flex flex-col md:flex-row items-center md:items-start gap-10 w-full md:w-auto">
        ...
        <div className="flex flex-col flex-1 justify-center max-w-2xl text-center md:text-left mt-4 md:mt-2">
          ...
          <div className="mb-10 mx-auto md:mx-0 max-w-lg">
            <p className="text-[11px] tracking-[0.2em] uppercase text-on-surface-variant/70 mb-3">Interests</p>
            {parsedInterests.length > 0 ? (
              ...
              <p className="text-sm text-on-surface-variant/70">No interests added yet.</p>
            )}
          </div>
